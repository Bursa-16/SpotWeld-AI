...
def _response_from_revision(
    *,
    ...
    return DigitalWeldPassportResponse(
        decision_outcome=current_state.value,
        result_type=current_result_type,
        result_id=revision.passport_id,
        result_revision=str(revision.revision_number),
...
def _load_exact_revision_response(
    *,
    ...
    current = session.scalar(
        select(DigitalWeldPassportLifecycleEvent).where(
            DigitalWeldPassportLifecycleEvent.passport_revision_id == revision.id
        )
    )
    if current is None:
        return None
